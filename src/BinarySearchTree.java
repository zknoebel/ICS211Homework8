...
public class BinarySearchTree<E> extends BinaryTree implements SearchTree<E> {
	 ...
	 * 
	 * @param localRoot:
	 *            the root node for the tree to be traversed
	 * @param item:
	 *            the object to be added to the tree
	 * @return: returns the root of the tree after "item" has been added
	 * 
	 *          this method recursively traverses the tree to find the spot in
	 *          which the new object, "item" should be added, then it creates a
	 *          new node containing "item" and places this node in the
	...
	/**
	 * uses the find() method to determine whether or not the tree contains a
	 * node containing "item" returns true if it does and false if it does not
	 */
	@Override
	...
	/**
	 * public starter method for the private find() method sets the value of
	 * "root" for the private find() method and then returns whatever the
	 * private method does
	 */
	@Override
	 ...
	 * @return: returns the object that matches "target" unless there is not one
	 *          in the tree, then it returns null
	 * 
	 *          compares "localRoot" to "target" if "localRoot" = "target"
	 *          returns local "root.data" if "localRoot" < "target" searches the
	 *          subtree with the root "localRoot.right.data" if "localRoot" >
	 *          "target" searches the subtree with the root
	 *          "localRoot.left.data" if "localRoot" is null, then return null
	 */
	...
	/**
	 * Starter method for Node<E> delete() changes the tree to a tree with the
	 * "target" deleted then returns the data in the deleted node
	 */
	@Override
	 ...
	 * 
	 * @param localRoot:
	 *            the root of the tree that contains the "target" to be deleted
	 * @param target:
	 *            an object that matches the object to be deleted
	 * @return: the object that was removed from the tree or null if nothing
	 *          matches the target
	 * 
	 *          recursive checks for a "localRoot.data" that matches "target" if
	 *          "localRoot.data" matches "target" remove localRoot if
	 *          "localRoot.data" > "target" check the subtree of the current
	 *          tree who's root is "localRoot.left" if "localRoot.data" <
	 *          "target" check the subtree of the current tree who's root is
	 *          "localRoot.right"
	 * 
	 *          if an inner node is deleted, moves all of its children to the
	 *          correct places
	 */
	 ...
	 * 
	 *          this is used in the delete() method to move nodes up if an inner
	 *          node is deleted
	 */
	...
	/**
	 * @return: a string of all of the nodes of the tree concatenated in a
	 *          preorder format
	 */
	 ...
	 * @param sb:
	 *            the string builder used to put all of the strings together
	 * 
	 *            uses a string builder to append all of the nodes together in
	 *            an inorder format separated by "\n" to make each one come out
	 *            on a separate line when the string is printed
	 */
	 ...
	 * 
	 * @param node:
	 *            the node who's data will appended to the string
	 * @param depth:
	 *            the level of the current node this is used to see how many
	 *            spaces should be before each data when the string is printed
	 * @param sb:
	 *            the string builder that will append everything
	 * 
	 *            uses a string builder to create a string of all of the data in
	 *            the tree in a preorder format
	 */
	...
	/**
	 * removes an item from the tree and then makes that node's children move to
	 * their new spots
	 * 
	 * return: false if the "target" does not exist or true if the "target" is
	 * removed
	 */
	@Override
